...
class _baseNetwork:
    ...
    def softmax(self, scores):
        ...
        sum = np.sum(scores_exp, axis=1).reshape(-1, 1)
        prob = scores_exp / sum
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def cross_entropy_loss(self, x_pred, y):
        ...
        #############################################################################
        x_row = x_pred.shape[0]
        x_col = x_pred.shape[1]
        x_pred_log = np.log(x_pred)

        y_idx = [idx * x_col + y_val for idx, y_val in enumerate(y)]
        y_act = np.zeros_like(x_pred).ravel()
        np.put(y_act, y_idx, 1)
        y_act = y_act.reshape((x_row, x_col))
        loss = - np.sum(x_pred_log * y_act) / x_row
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return loss
    ...
    def compute_accuracy(self, x_pred, y):
        ...
        accurate = np.equal(x_pred_class, y)
        acc = np.sum(accurate) / accurate.shape[0]
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return acc
    ...
    def ReLU_dev(self, X):
        ...
        #############################################################################
        ReLU_dev = lambda x: 1 if x > 0 else 0
        vfunc = np.vectorize(ReLU_dev)
        out = vfunc(X)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
