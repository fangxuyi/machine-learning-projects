...
class SoftmaxRegression(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        #############################################################################
        X = X.reshape(len(X), -1)
        x_mult = np.matmul(X, self.weights['W1'])
        x_activated = self.ReLU(x_mult)
        x_pred = self.softmax(x_activated)
        ...
        gradient = - 1 / x_pred.shape[0] * np.matmul(X.transpose(), (y_act - x_pred) * self.ReLU_dev(x_mult))
